...
signed main(){
  ...
  long long tt=1;
  for(int i=0;i<=n;i++){
    aa.xi[i]=pow(2,((1ll*i*(i-1))/2)%(mod-1))%mod;
    aa.xi[i]=(1ll*aa.xi[i]*tt)%mod;
    tt*=pow(i+1,mod-2);
    tt%=mod;
  }
  tt=1;
  for(int i=1;i<=n;i++){
    bb.xi[i]=pow(2,((1ll*i*(i-1))/2)%(mod-1))%mod;
    bb.xi[i]=(1ll*bb.xi[i]*tt)%mod;
    tt*=pow(i,mod-2);
    tt%=mod;   
  }
  aa=inv(aa);
  cc=aa*bb;
  printf("%lld",1ll*cc.xi[n]*pow(tt,mod-2)%mod*pow(n,mod-2)%mod);
  return 0;
}
inline int read(){
