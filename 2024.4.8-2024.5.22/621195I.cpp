...
Poly FFT(const Poly &a,const Poly &b){
  ...
  int tol=1;
  int k=0;
  while(tol<std::min(a.ci+b.ci+2,2000002)){
    tol<<=1;
    k++;
  }
...
Poly NTT(const Poly &a,const Poly &b){
  ...
  ans.ci=a.ci+b.ci;
  for(int i=0;i<=std::min(a.ci+b.ci,2000001);i++){
...
Poly inv(Poly &a,Poly &ans2){
  ans2=Poly(a.ci);
  inv_work(ans2,a,a.ci+1);
  return ans2;
}
void Dao(Poly &b){
  if(b.ci==0){
...
void exp_work(Poly &ans,Poly &b,int len){
  ...
  for(int i=ans.ci+1;i<=std::min((len)*2,2000001);i++){
    ans.xi[i]=0;
  }
  ans.ci=len*2;
  a=ln(ans);
  ...
  ans=temp*ans;
  for(int i=len;i<=std::min(ans.ci,2000001);i++){
    ans.xi[i]=0;
  }
  ans.ci=len-1;
  return ;
}
Poly exp(Poly &a,Poly &ans){
...
#endif
Poly aa(2000001),bb(1000001),cc(700001),cc2(700001);
int fac[2000005];
int invv[2000005];
signed main(){
  #ifdef ONLINE_JUDGE
  ...
  aa=bb;
  bb.ci++;
  bb.ci=std::min(bb.ci,1000001);
  for(int i=bb.ci;i>=1;i--){
    bb.xi[i]=bb.xi[i-1];
  }
  bb.xi[0]=0;
  exp(bb,cc);
  bb=cc;
  cc.ci++;
  cc.ci=std::min(cc.ci,700001);
  for(int i=cc.ci;i>=1;i--){
    cc.xi[i]=cc.xi[i-1];
  }
  cc.xi[0]=0;
  cc=exp(cc,cc2);
  ...
  invv[2000001]=pow(fac[2000001],mod-2);
  ...
  while(T--){
    int s=read(),k=read();
    if(k==1){
      printf("%lld\n",1ll*aa.xi[s]*fac[s]%mod);
    }
    if(k==2){
      printf("%lld\n",1ll*bb.xi[s]*fac[s]%mod);
    }
    if(k==3){
      printf("%lld\n",1ll*cc2.xi[s]*fac[s]%mod);
    }
  }
  return 0;
}
