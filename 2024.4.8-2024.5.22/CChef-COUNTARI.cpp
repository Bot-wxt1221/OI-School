...
#include <complex>
#include <random>
#define CONFIG_POLY_DATA long long
#define CONFIG_FFT y
//#define CONFIG_NTT y
//#define mod 1945555039024054273
//#define g 5
//#define invg (1945555039024054273ll*3+1)/5
inline int pow(int aa,int k){
  __int128 a=aa;
  __int128 res=1;
  while(k){
    if(k&1){
  ...
  static void ntt(int *a,int lim,int type,int *r){
    ...
    for(int i=1;i<lim;i<<=1){
      ...
      for(int j=0;j<lim;j+=(i<<1)){
        ...
        for(int k=0;k<i;k++){
          int x=a[j+k];
          int y=1ll*(__int128)a[i+j+k]*g0%mod;
          a[j+k]=(x+y)%mod;
          a[i+j+k]=(((x-y)%mod)+mod)%mod;
          g0=(1ll*(__int128)g0*omega)%mod;
        }
      }
    }
    return ;
...
Poly NTT(const Poly &a,const Poly &b){
  int tol=1;
  int k=0;
  while(tol<std::min(a.ci+b.ci+2,(int)1000002)){
    tol<<=1;
    k++;
  } 
  ans.ci=0;
...
signed main(){
  ...
  int ku=std::sqrt((long long)(n))*2;
  long long ans=0;
  for(int i1=1,i2;i1<=n;i1=i2+1){
    i2=std::min(i1+ku-1,n);
    for(int k=i1;k<=i2;k++){
      bb.xi[aa[k]]--;
    }
    dd=bb*cc;
    for(int k=i1;k<=i2;k++){
      ans+=dd.xi[aa[k]*2];
        for(int kk2=k+1;kk2<=i2;kk2++){
        if(2*aa[k]-aa[kk2]>=0){
          ans+=cc.xi[2*aa[k]-aa[kk2]];
        }
      }
      cc.xi[aa[k]]++;
    }
    for(int k=i2;k>=i1;k--){
      for(int kk2=i1;kk2<k;kk2++){
        if(2*aa[k]-aa[kk2]>=0){
          ans+=bb.xi[2*aa[k]-aa[kk2]];
        }
      }
    }
  }
