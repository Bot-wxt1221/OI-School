...
signed main(){
  ...
  int suma2=0,sumb2=0;
  for(int i=0;i<n;i++){
    ta.xi[i]=ta.xi[i+n]=read();
    suma+=ta.xi[i];
    suma2+=ta.xi[i]*ta.xi[i];
    maxc=std::min(maxc,m-ta.xi[i]);
  }
  int minc=-0x3f3f3f3f;
  tb.xi[0]=0;
  for(int i=1;i<=n;i++){
    tb.xi[i]=read();
    sumb+=tb.xi[i];
    sumb2+=tb.xi[i]*tb.xi[i];
    minc=std::max(minc,tb.xi[i]-m);
  }
  std::reverse(tb.xi+1,tb.xi+n+1);
  long long minans=0x3f3f3f3f3f3f3f3f;
  for(int i=-10000;i<=10000;i++){
    minans=std::min(minans,1ll*2*(suma-sumb)*i+1ll*n*i*i);
  }
  ta=ta*tb;
  int minans2=-0x3f3f3f3f;
  for(int i=n;i<2*n;i++){
    minans2=std::max(minans2,ta.xi[i]);
  }
  printf("%lld",minans-2*minans2+suma2+sumb2);
  return 0;
}
inline int read(){
