...
Poly FFT(const Poly &a,const Poly &b){
  ...
  int tol=1;
  int k=0;
  while(tol<std::min(a.ci+b.ci+2,100002ll)){
    tol<<=1;
    k++;
  }
...
void dfs(int now,int fa){
  ...
  long long cur2=1;
  for(int i=0;i<=k;i++){
    dp[now].xi[i]=(cur*1ll*cur2)%mod;
    dp[now].xi[i]*=p[now];
    dp[now].xi[i]%=mod;
    cur*=1ll*((aa[now]-i)%mod+mod)%mod;
    cur%=mod;
    cur2*=modd::pow((i+1),mod-2);
    cur2%=mod;
  }
  for(int i=fir[now];i!=-1;i=nxt[i]){
    ...
    dfs(v[i],now);
    Poly ans=dp[now]*dp[v[i]];
    for(int j=0;j<=k;j++){
      dp[now].xi[j]=dp[now].xi[j]*(1-p[v[i]]+mod)%mod;
      dp[now].xi[j]+=ans.xi[j];
      dp[now].xi[j]%=mod;
    }
  }
  return ;
...
signed main(){
  ...
  for(int i=1;i<=k;i++){
    for(int j=1;j<=k;j++){
      c[i][j]=(c[i-1][j]*j%mod+c[i-1][j-1])%mod;
    }
  }
  fa[1]=0;
  p[0]=0;
  ...
  for(int i=1;i<=n;i++){
    int tt=1;
    for(int j=0;j<=k;j++){
      dp[i].xi[j]*=(1-p[fa[i]]+mod)%mod;
      dp[i].xi[j]%=mod;
      tans+=dp[i].xi[j]*tt%mod*c[k][j]%mod;
      tans%=mod;
      tt*=(j+1);
      tt%=mod;
    }
